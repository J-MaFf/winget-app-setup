...
installed" — a structural runner-image defect, not a bug in this repo. Two complementary
mitigations have landed: `.github/workflows/e2e-install.yml`'s `e2e-install` job is pinned off the
rolling `windows-latest` label to `windows-2022` (the last known-good major version) to avoid the
broken image outright, and `Test-AppxMissingFrameworkDependency`
...
this same conflict is observed mid-run. **The issue stays open**: pinning mitigates the symptom by
avoiding the broken image, but does not fix the underlying runner-image defect — closing it needs
a real `e2e-install` run on `windows-2022` (next scheduled run, or `gh workflow run
e2e-install.yml`) confirming the idempotence pass completes.
...
| `.github/workflows/e2e-install.yml` | E2E tier 1: weekly real install run on GitHub-hosted `windows-2022` (pinned off `windows-latest`, issue #279; schedule + dispatch + self-validating PRs; failure auto-files an issue) |
...
- Confirm [#279](https://github.com/J-MaFf/winget-app-setup/issues/279) is actually resolved by running `e2e-install` on the now-pinned `windows-2022` label (next scheduled Monday run, or `gh workflow run e2e-install.yml`) and checking the idempotence (second) pass completes; close the issue once confirmed, and periodically re-check whether `windows-latest`/Server 2025's runner image has been fixed upstream.
